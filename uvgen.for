...
c	can be "altaz", "nasmyth" or "equatorial". The second value gives the feed
...
c@ leakoff
c   Polarization leakage offset, given as a percent. This gives the
c   real and imaginary part off the offsets. These are added to the rms
c   values defined in leakage above. If you set leakoff you must set
c   leakage as well.
c   To use this, you must set
c     stokes=xx,yy,xy,yx
c   or
c     stokes=rr,ll,rl,lr
c   The default is 0,0 (i.e. no polarization leakage offset).
c@ zeeman
...
c    29feb24  rr   add leakage offset and mount NASMYTH
c
c  Bugs/Shortcomings:
...
c
	...
	if(telescop.eq.'ALTAZ')then
	  ...
	  mount = EQUATOR
	  dtemp = 0
	else if(telescop.eq.'NASMYTH')then
	  mount = NASMYTH
      dtemp = 0
	else
...
c
	...
	leakrms = leakrms / 100.
	call keyr('leakoff',leakre,0.)
	call keyr('leakoff',leakim,0.)
	leakre = leakre / 100.
	leakim = leakim / 100.
...
c
	if(doleak)then
	  ...
	  do i=1,nant
	    Leak(1,i) = leakrms * leak(1,i) + complex(leakre,leakim)
	    Leak(2,i) = leakrms * leak(2,i) + complex(leakre,-leakim)
...
c
	    if(mount.eq.ALTAZ)then
          ...
          psi = atan2(sinq,cosq) + evector - elev
	    else
	      psi = evector
	    endif
